...
test(`has returns true forall x element of S`, () => {
  F.assert(F.property(
    F.array(F.anything()), F.anything(), F.array(F.anything()),
    (init, x, tail) => ok(has(x, of(...[...init, x, ...tail])))
  ))
})
...
test('union has empty identity', () => {
  F.assert(F.property(
    immutableSet(F.anything()),
    s => ok(equal(s, union(s, empty)))
  ))
})

test('union is associative', () => {
  F.assert(F.property(
    immutableSet(F.anything()), immutableSet(F.anything()), immutableSet(F.anything()),
...
test('union is commutative', () => {
  F.assert(F.property(
    immutableSet(F.anything()), immutableSet(F.anything()),
    (s1, s2) => ok(equal(union(s1, s2), union(s2, s1)))
  ))
})
...
test('intersection is associative', () => {
  F.assert(F.property(
    immutableSet(F.anything()), immutableSet(F.anything()), immutableSet(F.anything()),
    (s1, s2, s3) => ok(equal(intersection(s1, intersection(s2, s3)), intersection(intersection(s1, s2), s3)))
  ))
})
...
test('intersection is commutative', () => {
  F.assert(F.property(
    immutableSet(F.anything()), immutableSet(F.anything()),
    (s1, s2) => ok(equal(intersection(s1, s2), intersection(s2, s1)))
  ))
})
...
test('difference has empty identity', () => {
  F.assert(F.property(
    immutableSet(F.anything()),
    s => ok(equal(s, difference(s, empty)))
  ))
})
